...
Before 2008, banks traded in the interbank fed funds market. If the fed wanted to ease monetary conditions it would use an open market purchase (growth too low and inflation not a threat), it would add reserves to the banking system, decreasing fed funds rate and increase volume of bank loans.
...
For administrative rates, the interest on reserve balances (IORB) and reverse repo facility (RRP). The fed sets a rated range for fed funds rate; it sets the IORB to determine the rate which banks are willing, it sets the RRP rate as the floor to the rate at which money market funds and other will lend.
...
- APs can also reduce the number of outstanding ETF shares by taking assets from the underlying portfolio, selling the, and purchasing existing ETF shares corresponding to the sold assets.
Investors cannot exchange their shares for securities in the underlying portfolio or deliver securities in exchange for new ETF shares.
